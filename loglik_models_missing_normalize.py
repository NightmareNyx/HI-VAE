...
def loglik_real(batch_data, list_type, theta, normalization_params, tau2, kernel_initializer, name, reuse):
    ...
    # Compute loglik
    log_p_x = -0.5 * tf.reduce_sum(tf.squared_difference(data, est_mean) / est_var, 1) - int(
        list_type['dim']) * 0.5 * tf.math.log(2 * np.pi) - 0.5 * tf.reduce_sum(tf.math.log(est_var), 1)
...
def loglik_pos(batch_data, list_type, theta, normalization_params, tau2, kernel_initializer, name, reuse):
    ...
    # Affine transformation of the parameters
    est_mean = tf.math.sqrt(data_var_log) * est_mean + data_mean_log
    ...
    output['log_p_x'] = tf.math.multiply(log_p_x, missing_mask)
    output['log_p_x_missing'] = tf.math.multiply(log_p_x, 1.0 - missing_mask)
    output['params'] = [est_mean, est_var]
    output['samples'] = tf.clip_by_value(
        tf.exp(tfp.distributions.Normal(est_mean, tf.sqrt(est_var)).sample()) - 1.0, 0, 1e20)
...
def loglik_cat(batch_data, list_type, theta, normalization_params, tau2, kernel_initializer, name, reuse):
    ...
    output['samples'] = tf.one_hot(tfp.distributions.Categorical(probs=tf.nn.softmax(log_pi)).sample(),
...
def loglik_ordinal(batch_data, list_type, theta, normalization_params, tau2, kernel_initializer, name, reuse):
    ...
    theta_values = tf.math.cumsum(tf.clip_by_value(tf.nn.softplus(partition_param), epsilon, 1e20), 1)
    sigmoid_est_mean = tf.nn.sigmoid(theta_values - mean_value)
    mean_probs = tf.concat([sigmoid_est_mean, tf.ones([batch_size, 1], tf.float32)], 1) - tf.concat(
        [tf.zeros([batch_size, 1], tf.float32), sigmoid_est_mean], 1)
    ...
    # Compute loglik
    log_p_x = -tf.nn.softmax_cross_entropy_with_logits_v2(logits=tf.math.log(mean_probs),
                                                          labels=tf.stop_gradient(true_values))

    output['log_p_x'] = tf.math.multiply(log_p_x, missing_mask)
    output['log_p_x_missing'] = tf.math.multiply(log_p_x, 1.0 - missing_mask)
    output['params'] = mean_probs
    output['samples'] = tf.sequence_mask(
        1 + tfp.distributions.Categorical(logits=tf.math.log(tf.clip_by_value(mean_probs, epsilon, 1e20))).sample(),
...
def loglik_count(batch_data, list_type, theta, normalization_params, tau2, kernel_initializer, name, reuse):
    ...
    log_p_x = -tf.reduce_sum(tf.nn.log_poisson_loss(targets=data, log_input=tf.math.log(est_lambda),
